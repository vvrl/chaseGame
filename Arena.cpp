...
Arena::~Arena() {
    ...
    //Проверить правильность освобождения памяти

    //Память в любом случае освобождается и передается в пользование операционной системе, полсе особождения выделенная ранее память заполняется мусором
    ...
    delete[] field;
    // Возможный вариант проверки. если указатель указывает в nullptr, это является сигналом, что он не указывает на участок памяти. В противном случае указывает.
